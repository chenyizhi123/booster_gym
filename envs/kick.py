...
class kick(BaseTask):
    ...
    def _init_buffers(self):
        ...
        self.goal_position[:, 2] = self.env_origins[:, 2] + 0.4
        ...
        # 默认目标点：球门中心
        default_target = self.goal_position.clone()
        self.shot_target_points[:] = default_target
        
        # 射门轨迹预测相关参数
        self.gravity = 9.81  # 重力加速度
        self.air_resistance = 0.01  # 简化的空气阻力系数
        # ==========================================
        ...
        self.field_indices = torch.arange(2,self.num_actors*self.num_envs, self.num_actors,device=self.device)
        # =============上面是机器人和足球的索引处理========================
        ...
        # if self.cfg["commands"]["cur_random"]:
        #     # 如果启用了球课程学习，则初始化课程等级和成功计数
        #     probs = torch.tensor([1, 3, 8, 8], dtype=torch.float32, device=self.device)
        #     self.ball_curriculum_level = torch.randint(0, 4, (self.num_envs,), device=self.device)
        #     self.ball_curriculum_level = torch.multinomial(probs, self.num_envs, replacement=True)  # 随机初始化课程等级
        # else:
        #     # 如果没有启用课程学习，则将所有环境的课程等级设置为3
        #     self.ball_curriculum_level.fill_(2)
        ...
        #     1: {"min_dist": 2.5, "max_dist": 4.5, "success_threshold": 15},  # 中距离  
        ...
        # # 成功条件配置
        # self.success_distance_threshold = self.cfg['rewards']['success_distance_threshold']  # 机器人到球的距离阈值(米)
        ...
        # 左上 (0): x不变, y不变, z=上半部分
        self.target_zones[:, 0, :] = goal_center
        self.target_zones[:, 0, 0] -= goal_width * 0.3  # 左侧30%位置
        self.target_zones[:, 0, 2] += goal_height * 0.25  # 上半部分
        
        # 左下 (1): x不变, y不变, z=下半部分  
        self.target_zones[:, 1, :] = goal_center
        self.target_zones[:, 1, 0] -= goal_width * 0.3  # 左侧30%位置
        self.target_zones[:, 1, 2] -= goal_height * 0.1  # 下半部分（稍微离地面）
        
        # 右上 (2)
        self.target_zones[:, 2, :] = goal_center  
        self.target_zones[:, 2, 0] += goal_width * 0.3  # 右侧30%位置
        self.target_zones[:, 2, 2] += goal_height * 0.25  # 上半部分
        
        # 右下 (3)
        self.target_zones[:, 3, :] = goal_center
        self.target_zones[:, 3, 0] += goal_width * 0.3  # 右侧30%位置  
        self.target_zones[:, 3, 2] -= goal_height * 0.1  # 下半部分
    ...
    def _reset_root_states(self, env_ids):
        ...
        # 重置足球的线性速度和角速度为零
        self.root_states[self.soccer_actor_indices, 7:13] = 0.0
    ...
    def _reward_shoot_line(self):
        ...
        if not valid_mask.any():
            return reward
        # 计算速度方向与目标方向的余弦相似度
        ball_velocity_unit = ball_velocity[valid_mask] / ball_speed[valid_mask].unsqueeze(-1)
        ball_to_target_unit = ball_to_target[valid_mask] / target_distance[valid_mask].unsqueeze(-1)
        # 余弦相似度 (-1到1，1表示完全一致)
        cos_similarity = torch.sum(ball_velocity_unit * ball_to_target_unit, dim=1)
